...
export class EventService {
  ...
  update(event: IEvent): Observable<EntityResponseType> {
    return this.http.put<IEvent>(`${this.resourceUrl}/${getEventIdentifier(event) as string}`, event, { observe: 'response' });
  }

  partialUpdate(event: IEvent): Observable<EntityResponseType> {
    return this.http.patch<IEvent>(`${this.resourceUrl}/${getEventIdentifier(event) as string}`, event, { observe: 'response' });
  }
